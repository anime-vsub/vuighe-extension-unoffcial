<template>
	...
	<div class="select" @click.prevent.stop="showOptions = !showOptions">
		...
		<ul class="options"  @click.prevent.stop="closeAfterSelect ? (showOptions = false) : void 0" v-show="showOptions">
				<li v-for="(item, index) in options" :key="index" :class="{ 
...
const optionSelected = computed<{
	...
	return props.options.find(opt => ((opt.value ?? opt.name) === props.modelValue))
})
...
.select {
	...
	user-select: none;
			...
			&.active {
				...
				font-weight: 600;
				&, &:hover {
					background-color: rgba(#1976d2, .1)
				}
			}
		}
	}
